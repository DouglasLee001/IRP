...
struct TspCacheBase {
    ...
    virtual const Graph::Tour& get(const NodeList &nodes) const = 0;

    // return true is overwriting happens, otherwise a new entry is added.
    virtual bool set(const Graph::Tour &sln) = 0;
...
struct TspCache_BinTreeImpl : public TspCacheBase {
    ...
    virtual const Graph::Tour& get(const NodeList &nodes) const override {
        NodeSet containNode(nodeNum, false);
        for (auto n = nodes.begin(); n != nodes.end(); ++n) { containNode[*n] = true; }
        return get(containNode);
    }

    virtual bool set(const Graph::Tour &sln) override {
        return false;
    }
